  ...
  const fetchForecast = async () => {
    ...
    try {
      const res = await fetch("/api/regions/forecast", {
        ...
        body: JSON.stringify({ regionId, regionName, language }),
      });
      ...
      setError(t?.region?.error || "Gagal memuat data forecast. Coba lagi.");
  ...
  return (
    <div className="!fixed !inset-0 !bg-black/40 !backdrop-blur-sm !flex !items-center !justify-center !z-[10000] !p-4">
      <div className="!bg-white/95 !backdrop-blur-md !rounded-2xl !w-full !max-w-lg !max-h-[90vh] !overflow-y-auto !shadow-2xl !border !border-gray-200/50 !p-6">
        {/* Header — matching AI Analysis style */}
        <div className="!flex !items-start !justify-between !mb-5">
          <div>
            <div className="!flex !items-center !gap-2.5 !mb-2">
              <MetaIcon size={22} className="!text-[#F55036]" />
            ...
            {data && (
              <div className="!inline-flex !px-3 !py-1 !rounded-full !text-sm !font-semibold !bg-blue-50 !text-blue-700 !border !border-blue-200">
                {t?.region?.climateData || "Data Iklim"} {data.climate.month} {data.climate.year}
              </div>
            )}
          </div>
          <button
        ...
        {loading ? (
          <div className="!flex !flex-col !items-center !justify-center !py-10">
            <LottieLoader size={100} text={t?.region?.analyzing || "Menganalisis data iklim..."} />
          </div>
        ) : error ? (
          ...
          <div className="!space-y-4">
            ...
            <div className="!p-4 !bg-gradient-to-r !from-gray-50 !to-slate-50 !rounded-xl !border !border-gray-200">
              <h4 className="!text-sm !font-bold !text-gray-800 !mb-3 !tracking-wide !uppercase" style={{fontSize: '11px', letterSpacing: '0.05em'}}>{t?.region?.sensorData || "Data Sensor Iklim"}</h4>
              <div className="!grid !grid-cols-3 !gap-3">
                {/* Temperature */}
                <div className="!text-center">
                  <div className="!flex !items-center !justify-center !gap-1 !mb-1">
                    <Thermometer size={13} className="!text-orange-500" />
                    <span className="!text-xs !font-semibold !text-gray-500">{t?.region?.temperature || "Suhu"}</span>
                  </div>
                ...
                <div className="!text-center">
                  <div className="!flex !items-center !justify-center !gap-1 !mb-1">
                    ...
                    <span className="!text-xs !font-semibold !text-gray-500">{t?.region?.humidity || "Kelembaban"}</span>
                  </div>
                ...
                <div className="!text-center">
                  <div className="!flex !items-center !justify-center !gap-1 !mb-1">
                    ...
                    <span className="!text-xs !font-semibold !text-gray-500">{t?.region?.wind || "Angin"}</span>
                  </div>
            ...
            {/* AI Prediction — matching Analysis card style */}
            <div className="!p-4 !bg-white !rounded-xl !border !border-gray-200">
              <h4 className="!text-sm !font-bold !text-gray-800 !mb-2 !tracking-wide !uppercase" style={{fontSize: '11px', letterSpacing: '0.05em'}}>{t?.region?.predictionTitle || "Prediksi Kualitas Udara"}</h4>
            ...
            <p className="!text-xs !text-gray-400 !text-center">
              {t?.region?.source || "Sumber data: BPS Kota Bandung — AI: Meta (Llama 4)"}
            </p>
          </div>
        ) : null}
      </div>
